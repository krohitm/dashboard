...
renderPlotly({
  dataToPlot1 <- filteredData() %>%
    select('date', ends_with('Actual')) %>%
    gather(key = "Department", value = "ActualRevenue", -date) %>%
    mutate(Department = gsub('Actual', '', Department)) %>%
  ...
  marker = list(
    ...
    layout(yaxis = list(title = 'Actual Revenue'))
  
  actualRev <- filteredData() %>%
    select(date, ends_with('Actual')) %>%
    gather(key = "DepartmentActual", value = "ActualRevenue", 
           contains('Actual')) %>%
    mutate(Department = gsub('Actual', '', DepartmentActual)) 
